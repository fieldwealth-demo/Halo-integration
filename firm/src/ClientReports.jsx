...
function PortfolioTrend() {
  ...
  const opts = React.useMemo(() => ({
    ...
    plotOptions:{
      area:{ marker:{ enabled:true, radius:4, lineWidth:2, lineColor:'rgb(14,26,42)' }, lineWidth:2.5 },
      series:{ states:{ hover:{ lineWidth:3 } } },
    },
    series:[
      {
        name:'Portfolio Value', data: portfolio, color:CR_GREEN,
        fillColor:{ linearGradient:{x1:0,y1:0,x2:0,y2:1}, stops:[[0,'rgba(35,89,255,0.28)'],[1,'rgba(35,89,255,0)']] },
      },
      {
        name:'S&P 500 Benchmark', data: benchmark, color:'rgb(163,163,163)',
        dashStyle:'Dash', fillColor:'rgba(0,0,0,0)',
        marker:{ enabled:true, radius:3, symbol:'circle', fillColor:'transparent', lineWidth:1.5, lineColor:'rgb(163,163,163)' },
      },
    ],
...
function AllocationDonut() {
  const data = [
    { name:'Domestic Stock Funds',   y:62, color:'rgb(168,185,241)',  amt:'$529,220' },
    { name:'Bond Funds',             y:18, color:'rgb(120,160,230)', amt:'$153,720' },
    { name:'International Equities', y:11, color:'rgb(180,150,235)', amt:'$93,940' },
    { name:'Cash',                   y:5,  color:'rgb(120,200,210)', amt:'$42,700' },
    { name:'Alternatives',           y:4,  color:'rgb(245,200,90)',  amt:'$34,160' },
  ];
  ...
  const opts = React.useMemo(() => ({
    ...
    plotOptions:{
      pie:{ innerSize:'68%',
        dataLabels:{ enabled:false }, states:{ hover:{ halo:null, brightness:0.05 } } },
    },
      ...
      <div style={{
        ...
        {data.map((d,i)=>(
          <div key={i} style={{ display:'flex', alignItems:'center', gap:6, fontFamily:'Inter', fontSize:11.5, color:'rgb(209,213,219)' }}>
            <span style={{ width:7, height:7, borderRadius:9999, background:d.color, flexShrink:0 }} />
...
function TransactionsTable() {
  ...
  const badge = (t) => {
    const map = {
      BUY:  { bg:CR_GREEN_SOFT, bd:'rgba(35,89,255,0.55)',  c:'rgb(140,175,255)' },
